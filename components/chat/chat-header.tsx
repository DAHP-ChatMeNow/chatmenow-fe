...
import { ChevronLeft, Phone, Video, MoreVertical, Image as ImageIcon, Link2, Search as SearchIcon, UserPlus, ShieldBan, Trash2, LogOut } from "lucide-react";
...
import { useMessages, useConversation, useAddMemberToGroup, useRemoveMemberFromGroup, useDissolveGroup } from "@/hooks/use-chat";
...
export function ChatHeader({ 
  ...
  const [sheetTab, setSheetTab] = useState<"media"|"links"|"search">("media");
  ...
  const partnerId = useMemo(() => {
    if (!conversation || conversation.type !== "private" || !currentUserId) return undefined;
    const partnerMember = conversation.members.find((m: any) => {
      const memberUserId = typeof m.userId === "string" ? m.userId : (m.userId?._id || m.userId?.id);
      return memberUserId !== currentUserId;
    });
    if (!partnerMember) return undefined;
    return typeof partnerMember.userId === "string" ? partnerMember.userId : (partnerMember.userId?._id || partnerMember.userId?.id);
  }, [conversation, currentUserId]);

  // Check if current user is admin in group
  const isAdmin = useMemo(() => {
    if (!conversation || conversation.type !== "group" || !currentUserId) return false;
  ...
  const groupMembers = useMemo(() => {
    if (!conversation || conversation.type !== "group" || !currentUserId) return [];
    return conversation.members
      ...
      .map((m: any) => ({
        userId: typeof m.userId === "string" ? m.userId : (m.userId?._id || m.userId?.id),
        displayName: typeof m.userId === "string" ? "Unknown" : (m.userId?.displayName || "Unknown"),
  ...
  return (
      ...
      <div className="flex items-center gap-2">
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end" className="w-60">
            ...
            <DropdownMenuItem onClick={() => { setSheetTab("media"); setSheetOpen(true); }}>
              <ImageIcon className="text-slate-500" /> Xem hình ảnh đã gửi
            </DropdownMenuItem>
            <DropdownMenuItem onClick={() => { setSheetTab("links"); setSheetOpen(true); }}>
              <Link2 className="text-slate-500" /> Xem link đã gửi
            </DropdownMenuItem>
            <DropdownMenuItem onClick={() => { setSheetTab("search"); setSheetOpen(true); }}>
            ...
            {conversation?.type === "private" && (
              <>
                ...
                >
                  <UserPlus className="text-slate-500" /> Tạo nhóm với người này
                </DropdownMenuItem>
                <DropdownMenuItem
                  onClick={() => {
                    if (partnerId) {
              ...
              <>
                ...
                {isAdmin && (
                  <>
                    ...
                    <DropdownMenuItem onClick={() => setDissolveConfirmOpen(true)}>
    ...
    <MessagesSideSheet
      ...
      title={sheetTab === "media" ? "Hình ảnh đã gửi" : sheetTab === "links" ? "Liên kết đã gửi" : "Tìm kiếm tin nhắn"}
    />
    <CreateGroupWithPartnerDialog
      ...
      onCreate={() => {
        ...
        const base = partnerId ? [partnerId, ...selectedIds.filter((x) => x !== partnerId)] : selectedIds;
        if (!groupName || base.length < 2) return;
        createGroupMutation.mutate({ name: groupName, memberIds: base }, {
          onSuccess: () => {
            setGroupOpen(false);
            setGroupName("");
            setSelectedIds(partnerId ? [partnerId] : []);
    ...
    <InviteMembersDialog
      ...
      onInvite={() => {
        ...
        addMemberMutation.mutate({ conversationId: currentId, memberIds: inviteSelectedIds }, {
          onSuccess: () => {
            setInviteOpen(false);
    ...
    <Dialog open={dissolveConfirmOpen} onOpenChange={setDissolveConfirmOpen}>
      <DialogContent>
        ...
        <p className="text-sm text-slate-600">Bạn có chắc chắn muốn giải tán nhóm này? Tất cả tin nhắn sẽ bị xóa.</p>
        <DialogFooter>
          <Button variant="outline" onClick={() => setDissolveConfirmOpen(false)} disabled={dissolveMutation.isPending}>Hủy</Button>
          <Button
            variant="destructive"
            onClick={() => {
              if (!currentId) return;
...
function MessagesSideSheet({
  ...
  const mediaMsgs = useMemo(() => messages.filter(m => m.type === "image" || (m.attachments || []).some((a: any) => (a.fileType || "").startsWith("image"))), [messages]);
  const linkMsgs = useMemo(() => messages.filter(m => (m.content || "").match(urlRegex)), [messages]);
  ...
  return (
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent side="right" className="sm:max-w-md">
        ...
        {tab === "search" && (
          <div className="mt-3">
            <Input placeholder="Nhập từ khóa..." value={searchQuery} onChange={(e) => onSearchQueryChange(e.target.value)} />
            <div className="mt-2 text-xs text-slate-500">{searchMsgs.length} kết quả</div>
          </div>
        )}
          ...
          {tab === "media" && (
            <div className="grid grid-cols-3 gap-2 pr-3">
              {mediaMsgs.length === 0 ? (
                ...
                mediaMsgs.map((m, idx) => {
                  const img = (m.attachments || []).find((a: any) => (a.fileType || "").startsWith("image"));
                  const src = img?.url || m.content;
                  return (
                    <img key={m.id || idx} src={src} alt="image" className="w-full h-24 object-cover rounded-md border" />
                  );
                })
              )}
            </div>
          ...
          {tab === "links" && (
            <div className="space-y-2 pr-3">
              {linkMsgs.length === 0 ? (
                ...
                linkMsgs.map((m, idx) => {
                      const links = ((m.content || "").match(urlRegex) || []) as string[];
                  return (
                    <div key={m.id || idx} className="p-2 rounded-md border">
                          {links.map((l: string, i: number) => (
                            <a key={i} href={l} target="_blank" rel="noreferrer" className="text-blue-600 text-sm break-all">
                              {l}
                        </a>
                      ))}
                      <div className="text-xs text-slate-500 mt-1">{new Date(m.createdAt).toLocaleString()}</div>
                    </div>
                  );
                })
              )}
      ...
      <DialogContent>
        ...
        <div className="space-y-4">
          <div>
            ...
            <Input className="mt-2" placeholder="Nhập tên nhóm" value={groupName} onChange={(e) => onGroupNameChange(e.target.value)} />
          </div>
          <div>
            <label className="text-sm font-medium text-slate-700">Thành viên (đã chọn 1)</label>
                ...
                return (
                  <label key={id} className="flex items-center gap-3 px-3 py-2 hover:bg-slate-50">
                    <input
                      ...
                      checked={checked}
                      onChange={(e) => {
                        setSelectedIds((prev: string[]) => e.target.checked ? [...prev, id] : prev.filter((x) => x !== id));
                      }}
                    />
            ...
            <div className="mt-2 text-xs text-slate-500">Chọn thêm ít nhất 1 người</div>
          </div>
        </div>
        <DialogFooter>
          <Button variant="outline" onClick={() => onOpenChange(false)} disabled={creating}>Hủy</Button>
          <Button onClick={onCreate} disabled={disableCreate}>{creating ? "Đang tạo..." : "Tạo nhóm"}</Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  );
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent>
        ...
        <div>
          <label className="text-sm font-medium text-slate-700">Chọn bạn bè</label>
          <div className="mt-2 max-h-56 overflow-auto rounded-md border border-slate-200/80">
            {contacts.length === 0 ? (
        ...
        <DialogFooter>
          ...
          <Button onClick={onInvite} disabled={selectedIds.length === 0 || inviting}>
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent>
        ...
        <div className="space-y-2 max-h-64 overflow-auto">
          {members.length === 0 ? (
            <div className="text-sm text-slate-500">Không có thành viên khác</div>
          ) : (
            members.map((member) => (
              <div key={member.userId} className="flex items-center justify-between p-2 rounded-md border border-slate-200/80">
                <div className="flex items-center gap-3">
                  {member.avatar ? (
                  ...
                  <div className="text-sm text-slate-800">{member.displayName}</div>
                  {member.role === "admin" && <span className="text-xs bg-blue-100 text-blue-800 px-2 py-1 rounded">Admin</span>}
                </div>
        ...
        {confirmRemoveId && (
          <div className="p-3 rounded-md bg-red-50 border border-red-200">
            <p className="text-sm text-red-800 mb-2">Xóa thành viên này khỏi nhóm?</p>
            <div className="flex gap-2 justify-end">
              <Button size="sm" variant="outline" onClick={() => setConfirmRemoveId(null)} disabled={removing}>Hủy</Button>
              <Button
                size="sm"
                variant="destructive"
                onClick={() => {
